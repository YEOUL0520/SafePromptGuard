@echo off
chcp 65001 >nul
setlocal
cd /d "%~dp0.."

echo [SafePrompt Guard] 심사용 Docker 실행
echo.

docker version >nul 2>&1
if errorlevel 1 (
    echo Docker가 설치되어 있지 않거나 실행 중이 아닙니다.
    echo 먼저 scripts\install-docker.bat 을 실행하거나 Docker Desktop을 켜 주세요.
    echo 자세한 안내: docs\INSTALL_DOCKER.md
    exit /b 1
)

if not exist ".env.prod" (
    if exist ".env.prod.example" (
        echo .env.prod 가 없어 예시 파일을 복사합니다...
        copy /Y ".env.prod.example" ".env.prod" >nul
    ) else (
        echo .env.prod 파일이 필요합니다. .env.prod.example 을 복사해 주세요.
        exit /b 1
    )
)

echo 이미지 pull 중...
docker compose -f docker-compose.prod.yml --env-file .env.prod pull
if errorlevel 1 (
    echo pull 실패. 이미지가 GHCR에 없거나 네트워크/권한 문제일 수 있습니다.
    echo docs\PUBLISH_GHCR.md (팀) / docs\INSTALL_DOCKER.md (심사) 를 확인하세요.
    exit /b 1
)

echo 컨테이너 시작...
docker compose -f docker-compose.prod.yml --env-file .env.prod up -d
if errorlevel 1 exit /b 1

echo.
echo ========================================
echo  웹 UI:  http://localhost:8080
echo  API:    http://localhost:8001/api/health
echo ========================================
echo.
echo 종료: docker compose -f docker-compose.prod.yml --env-file .env.prod down
endlocal
